...
class WeatherService:
    ...
    def query_weather(self, city: str, extensions: str = "all") -> Dict[str, Any]:
        ...
        try:
            ...
            if data["status"] == "1":
                ...
                # 添加一个格式化的简要信息
                if extensions == "base":
                    lives = data.get("lives", [])   ##从API返回值中取出list字段
                    if lives:
    ...
    # 4. 格式化当前天气信息
    def _format_current_weather(self, weather: Dict[str, Any], city_name: str) -> str: ##str是键的类型，any是值
        return (
...
class WeatherTools:
    ...
    def query_weather(self, city: str) -> str:
        ...
        if result["status"] == "success" and "summary" in result:
            ...
            return f"获取{city}的天气信息失败: {result.get('message', '未知错误')}"   #如果message有信息，
                                                                                 #否则就返回未知错误