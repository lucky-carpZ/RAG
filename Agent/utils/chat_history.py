...
class ChatHistoryManager:
        ...
        try:
            if os.path.exists(HISTORY_FILE):
                with open(HISTORY_FILE, 'r', encoding='utf-8') as f:
                    return json.load(f)         ##将文件内容解析为一个python对象，返回
    ...
    def save_history(self) -> None:
        try:
            with open(HISTORY_FILE, 'w', encoding='utf-8') as f:  #将history写入文件f
                json.dump(self.history, f, ensure_ascii=False, indent=2)    ##indent表示首行缩进
        except Exception as e:
